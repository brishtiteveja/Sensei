...
> This is the original v1 Sensei Expo client. The shipping app now lives in `../mobile/`
> (the ShikkhaDikkha design import). Nothing here is wired into the build.
> Kept for two things worth reusing:
> - `src/api/stream.ts` — SSE over `XMLHttpRequest`, handles partial frames and aborts.
> - the box-health / offline-guard pattern in `src/state/store.tsx` (health poll ->
>   `online` flag consumed by `src/screens/ChatScreen.tsx`) and `src/api/http.ts`.
